...
def get_db():
    ...
    if encryption_key and SQLCIPHER_AVAILABLE:
        # V011 FIX: Validate encryption key format to prevent SQL injection
        # Only allow hex characters (as generated by secrets.token_hex())
        if not re.match(r'^[a-fA-F0-9]+$', encryption_key):
            raise ValueError(
                "DB_ENCRYPTION_KEY must contain only hexadecimal characters. "
                "Generate with: python -c \"import secrets; print(secrets.token_hex(32))\""
            )
        db = sqlcipher.connect(db_path)
        db.row_factory = sqlcipher.Row
        # Set encryption key (PRAGMA key must be first command)
        # Key is validated above to only contain hex chars, safe from injection
        db.execute(f"PRAGMA key = \"x'{encryption_key}'\"")  # Use hex key format
